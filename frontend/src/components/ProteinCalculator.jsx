...
import { Beef, Calculator, RotateCcw, Heart, AlertCircle, CheckCircle, Target, Download, FileText, Loader2, Info, Activity, Zap, TrendingUp, Sparkles, Utensils, ShieldCheck, Dumbbell } from "lucide-react";
  ...
  const calculateProtein = () => {
    if (!isFormValid()) {
      toast({
        ...
        description: "Please fill in all required fields to calculate your protein requirements.",
        variant: "destructive",
      });
      return;
    }
    ...
    setTimeout(() => {
      try {
        ...
        let multiplier;
        if (goal === "lose") multiplier = 2.2; // High protein for muscle preservation
        else if (goal === "gain") multiplier = 2.0; // Optimal for hypertrophy
        else if (goal === "strength") multiplier = 1.8;
        else multiplier = 1.4; // Active maintenance

        // Activity adjustment
        const activityAdj = parseFloat(activityLevel) >= 1.725 ? 0.2 : 0;
        const totalMultiplier = multiplier + activityAdj;
        
        const proteinGrams = wKg * totalMultiplier;
        const proteinCal = proteinGrams * 4;

        setResult({
          grams: Math.round(proteinGrams),
          calories: Math.round(proteinCal),
          perKg: totalMultiplier.toFixed(1),
          distribution: {
            breakfast: Math.round(proteinGrams * 0.25),
            lunch: Math.round(proteinGrams * 0.30),
            dinner: Math.round(proteinGrams * 0.30),
            snack: Math.round(proteinGrams * 0.15)
          },
          sources: [
            { name: "Chicken Breast", amount: "31g / 100g", category: "Lean Animal" },
            { name: "Greek Yogurt", amount: "10g / 100g", category: "Dairy" },
            { name: "Lentils", amount: "18g / cup", category: "Plant" },
            { name: "Whey Isolate", amount: "25g / scoop", category: "Supplement" }
          ]
        });
      ...
      <Card className="glass-panel glow-border border-white/10">
        <CardHeader className="text-center pb-8 border-b border-white/5 bg-white/[0.02]">
          <CardTitle className="text-4xl font-black mb-4 flex items-center justify-center gap-3">
            ...
            >
              <Beef className="h-10 w-10 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.4)]" />
            </motion.div>
          ...
          <p className="text-slate-400 text-lg max-w-xl mx-auto font-medium">
            Project your optimal protein nitrogen-balance requirements for hypertrophy and lean-mass preservation.
          </p>
        </CardHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:p-6 lg:p-8">
              <motion.div variants={itemVariants} className="space-y-6">
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Somatic Identifiers</h4>
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-500 ml-1 tracking-widest">Total Mass (Weight) *</Label>
              ...
              <motion.div variants={itemVariants} className="space-y-6">
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-500 ml-1 tracking-widest">Activity Gradient *</Label>
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-500 ml-1 tracking-widest">Anabolic Strategy *</Label>
                  <Select value={goal} onValueChange={setGoal}>
                    ...
                    <SelectContent className="glass-panel border-white/10">
                      ...
                      <SelectItem value="gain">Maximum Hypertrophy</SelectItem>
                      <SelectItem value="strength">Performance/Strength</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
          ...
          >
              ...
              <div className="p-6 sm:p-8 md:p-12 rounded-3xl sm:rounded-[3rem] lg:rounded-[4rem] bg-white/[0.03] border border-white/5 space-y-8">
                ...
                <div className="space-y-6">
                  <div className="p-6 rounded-[2rem] bg-white/5 border border-white/5 flex justify-between items-center group hover:bg-white/10 transition-all shadow-inner">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Energy Contribution</span>
                    <span className="text-2xl font-black text-white tracking-tighter">{result.calories} KCAL</span>
                  </div>
                  <div className="p-6 rounded-[2rem] bg-white/5 border border-white/5 flex justify-between items-center group hover:bg-white/10 transition-all shadow-inner">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Synthesis Ratio</span>
            ...
            <div className="mt-20 p-6 sm:p-10 md:p-14 rounded-3xl sm:rounded-[4rem] lg:rounded-[5rem] bg-emerald-500/5 border border-emerald-500/10 backdrop-blur-3xl relative overflow-hidden">
              ...
              <h4 className="text-2xl font-black text-white uppercase tracking-widest mb-10 flex items-center gap-4">
                ...
                Tissue Maintenance Protocols
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
                {[
                  "Optimize Leucine threshold (3g/meal).",
                  "Prioritize high-bioavailability sources.",
                  "Utilize slow-release proteins pre-sleep.",
                  "Hydrate excessively for nitrogen waste."
                ].map((rec, i) => (
            ...
            <div className="mt-24 text-center">
              <div className="bg-red-500/5 border border-white/5 p-6 sm:p-8 lg:p-10 rounded-3xl sm:rounded-[3rem] lg:rounded-[4rem] max-w-2xl mx-auto backdrop-blur-sm">
                ...
                <h5 className="text-2xl font-black text-white uppercase tracking-[0.2em] mb-4">Nitrogen Advisory</h5>
                <p className="text-slate-500 font-medium text-lg leading-relaxed lowercase tracking-tight">
                  "Exceeding 3.5g/kg for prolonged periods may strain renal filtration in predisposed individuals. always monitor metabolic markers."
                </p>
              </div>
            </div>
          </motion.div>
